...
case 'certificate':
	...
	exit('{"code":-1,"msg":"支付宝接口返回异常操作失败，请稍后重试"}');
		}
...
case 'printOrder':
	...
	try{
		...
		safe_error_log('exception', $e->getMessage());
	exit('{"code":-1,"msg":"打印失败：操作失败，请稍后重试"}');
	}
break;
case 'settle_result':
	$id=intval($_GET['id']);
...
case 'addDomain':
	...
	if(!$DB->exec("INSERT INTO `pre_domain` (`uid`,`domain`,`status`,`addtime`) VALUES (:uid, :domain, 0, NOW())", [':uid'=>$uid, ':domain'=>$domain]))safe_error_log('db_error', $DB->error());
...
case 'delDomain':
	if(!$conf['pay_domain_open']) exit('{"code":-1,"msg":"未开启授权支付域名添加"}');
	$id = intval($_POST['id']);
	if(!$DB->exec("DELETE FROM pre_domain WHERE id='$id' and uid='$uid'"))safe_error_log('db_error', $DB->error());
