...
case 'login':
	...
	// A2: CSRF 校验，使用 hash_equals 恒定时间比较
	if(!isset($_POST['csrf_token']) || !isset($_SESSION['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token']))exit('{"code":-1,"msg":"CSRF TOKEN ERROR"}');
	// A2: 登录限流 - IP、账号、IP+账号三维
	// 直接使用 REMOTE_ADDR，不信任伪造的 XFF/X-Real-IP
	$a2_real_ip = $_SERVER['REMOTE_ADDR'];
	$a2_limit_dir = sys_get_temp_dir() . '/epay_login_limit';
	@mkdir($a2_limit_dir, 0777, true);
	$a2_now = time();
	$a2_window = 300; // 5分钟窗口
	$a2_max_ip = 20; // IP维度最多20次失败
	$a2_max_user = 10; // 账号维度最多10次失败
	$a2_max_combo = 15; // IP+账号维度最多15次失败

	function a2_get_fail_count($dir, $key, $window, $now){
		$file = $dir . '/' . md5($key) . '.json';
		if(!file_exists($file)) return 0;
		$data = json_decode(file_get_contents($file), true);
		if(!$data || !isset($data['time']) || $now - $data['time'] > $window) return 0;
		return isset($data['count']) ? (int)$data['count'] : 0;
	}
	function a2_inc_fail_count($dir, $key, $now){
		$file = $dir . '/' . md5($key) . '.json';
		$data = ['time'=>$now, 'count'=>1];
		if(file_exists($file)){
			$old = json_decode(file_get_contents($file), true);
			if($old && isset($old['time']) && $now - $old['time'] < 300){
				$data['count'] = (int)$old['count'] + 1;
			}
		}
		file_put_contents($file, json_encode($data));
	}
	function a2_clear_fail_count($dir, $key){
		$file = $dir . '/' . md5($key) . '.json';
		if(file_exists($file)) @unlink($file);
	}

	$a2_fail_ip = a2_get_fail_count($a2_limit_dir, 'ip_'.$a2_real_ip, $a2_window, $a2_now);
	$a2_fail_user = a2_get_fail_count($a2_limit_dir, 'user_'.$user, $a2_window, $a2_now);
	$a2_fail_combo = a2_get_fail_count($a2_limit_dir, 'combo_'.$a2_real_ip.'_'.$user, $a2_window, $a2_now);

	if($a2_fail_ip >= $a2_max_ip || $a2_fail_user >= $a2_max_user || $a2_fail_combo >= $a2_max_combo){
		exit(json_encode(['code'=>-1,'msg'=>'登录失败次数过多，请5分钟后再试']));
	}
	...
	if($userrow && ($type==0 && $pass==$userrow['key'] || $type==1 && $pass==$userrow['pwd'])) {
		...
		ob_clean();
		session_regenerate_id(true); // A2: 登录成功轮换 Session ID
...
case 'reg':
	...
	if($conf['reg_pay']==1){
		...
		if($sds){
			...
			safe_error_log("订单创建失败", $DB->error());
		...
		if($sds){
			...
			safe_error_log("申请商户失败", $DB->error());
...
case 'findpwd':
	...
	if($sqs!==false){
		...
		safe_error_log('db_error', $DB->error());
