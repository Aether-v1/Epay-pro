...
<?php
	...
	if(!csrf_verify())exit('CSRF Token Error');
	...
	if(!empty($newpwd) && !empty($newpwd2)){
		...
		@$DB->insert('log', ['uid'=>0, 'type'=>'修改支付密码', 'date'=>'NOW()', 'ip'=>$clientip, 'city'=>'']);
	}else{
		showmsg('新密码不能为空',3);
	}
	$ad=$CACHE->clear();
	if($ad)showmsg('修改成功！',1);
	else { safe_error_log('修改失败', $DB->error()); showmsg('修改失败！',4); }
	...
	saveSetting('admin_user',$user);
		@$DB->insert('log', ['uid'=>0, 'type'=>'修改管理员账号', 'date'=>'NOW()', 'ip'=>$clientip, 'city'=>'']);
	if(!empty($newpwd) && !empty($newpwd2)){
		// A1: password_verify 验证旧密码
		$storedPwd = $conf['admin_pwd'];
		$oldPwdOk = false;
		if(strpos($storedPwd, '$2y$') === 0 || strpos($storedPwd, '$2a$') === 0 || strpos($storedPwd, '$argon2') === 0){
			$oldPwdOk = password_verify($oldpwd, $storedPwd);
		} elseif($storedPwd === $oldpwd && $storedPwd !== '!INSTALL_REQUIRED!'){
			$oldPwdOk = true;
		}
		if(!$oldPwdOk)showmsg('旧密码不正确！',3);
		if($newpwd!=$newpwd2)showmsg('两次输入的密码不一致！',3);
		if(strlen($newpwd) < 8)showmsg('新密码不能少于8位！',3);
		if(!preg_match('/[A-Za-z]/', $newpwd) || !preg_match('/[0-9]/', $newpwd))showmsg('新密码必须包含字母和数字！',3);
		if(strlen($newpwd) > 100)showmsg('新密码位数过长！',3);
		saveSetting('admin_pwd', password_hash($newpwd, PASSWORD_DEFAULT));
		@$DB->insert('log', ['uid'=>0, 'type'=>'修改管理员密码', 'date'=>'NOW()', 'ip'=>$clientip, 'city'=>'']);
	}
	$ad=$CACHE->clear();
...
<div class="panel-body">
  <form action="./set.php?mod=account_n" method="post" class="form-horizontal" role="form"><input type="hidden" name="do" value="submit"/><?= csrf_field() ?>
	<div class="form-group">
...
<div class="panel-body">
  <form action="./set.php?mod=paypwd_n" method="post" class="form-horizontal" role="form"><input type="hidden" name="do" value="submit"/><?= csrf_field() ?>
	<div class="form-group">
	  <label class="col-sm-2 control-label">旧密码</label>
...
if(!in_array($file_ext, $allowed_ext)){
	echo "上传失败，仅支持图片文件（gif/jpg/png/bmp/webp/svg）";
}elseif($_FILES['file']['size'] > 2 * 1024 * 1024){
	echo "上传失败，文件大小不能超过2MB";
}else{
$allowed_mime = ['image/gif', 'image/jpeg', 'image/png', 'image/bmp', 'image/webp', 'image/svg+xml'];
$file_mime = function_exists('finfo_file') ? finfo_file(finfo_open(FILEINFO_MIME_TYPE), $_FILES['file']['tmp_name']) : $_FILES['file']['type'];
if(!in_array($file_mime, $allowed_mime)){
	echo "上传失败，文件类型不允许，仅支持图片文件";
}elseif(copy($_FILES['file']['tmp_name'], ROOT.'assets/img/logo.png')){
