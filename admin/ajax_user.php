...
case 'delGroup':
	...
	else safe_error_log('db_error', $DB->error());
	exit('{"code":-1,"msg":"删除用户组失败"}');
break;
case 'saveGroup':
	if($_POST['action'] == 'add'){
...
break;
case 'editUser':
	// Batch C Phase2: 修改商户权限检查和审计
	require_permission('user.edit');
	admin_audit_log('修改商户', json_encode(['uid'=>$_POST['uid'] ?? '']));
	$uid=intval($_GET['uid']);
...
break;
case 'delUser':
	// Batch C Phase2: 删除商户权限检查和审计
	require_permission('user.delete');
	admin_audit_log('删除商户', json_encode(['uid'=>$_POST['uid'] ?? '']));
	$uid=intval($_GET['uid']);
...
case 'recharge':
	...
	// Batch D Phase1: 金额校验（拒绝负数、零、非数字）
	if($rmb <= 0 || !is_numeric($_POST['rmb']) || !preg_match('/^[0-9.]+$/', $_POST['rmb']))exit('{"code":-1,"msg":"金额不合法"}');
...
case 'addDomain':
	...
	if(!$DB->exec("INSERT INTO `pre_domain` (`uid`,`domain`,`status`,`addtime`,`endtime`) VALUES (:uid, :domain, 1, NOW(), NOW())", [':uid'=>$uid, ':domain'=>$domain]))safe_error_log('db_error', $DB->error());
