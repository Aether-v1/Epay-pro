...
while($row = $rs->fetch()){
	...
	$file.='="'.$row['trade_no'].'",="'.safe_csv($row['out_trade_no']).'",="'.$row['api_trade_no'].'",'.$row['uid'].','.safe_csv($row['domain']).','.text_encoding(safe_csv($row['name'])).','.$row['money'].','.$row['realmoney'].','.$row['getmoney'].','.$paytype[$row['type']].','.$row['channel'].','.$row['plugin'].','.$row['buyer'].','.$row['ip'].','.$row['addtime'].','.$row['endtime'].','.display_status($row['status']).','.($row['status']==2?$row['refundmoney']:'').','.$row['refundtime']."\r\n";
}
