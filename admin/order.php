		...
		columns: [
			...
			},
			{
				...
				formatter: function(value, row, index) {
					return '<a href="javascript:showOrder(\''+value+'\')" title="点击查看详情">'+value+'</a></b><br/>'+$('<div/>').text(row.out_trade_no).html();
				}
			},
			{
				field: 'uid',
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				item += '<tr><td class="info" class="orderTitle">商户订单号</td><td colspan="5" class="orderContent">'+$('<div/>').text(data.out_trade_no).html()+'</td></tr>';
