...
case 'set':
	if(isset($_POST['localurl'])){
		...
		if(!empty($_POST['localurl']) && !ssrf_validate_url($_POST['localurl']))exit('{"code":-1,"msg":"回调专用网址指向内网或保留地址，已被安全策略拦截"}');
	}
	if(isset($_POST['apiurl'])){
		if(!empty($_POST['apiurl']) && (substr($_POST['apiurl'],0,4)!='http' || substr($_POST['apiurl'],-1)!='/'))exit('{"code":-1,"msg":"用户对接网址格式错误"}');
		if(!empty($_POST['apiurl']) && !ssrf_validate_url($_POST['apiurl']))exit('{"code":-1,"msg":"用户对接网址指向内网或保留地址，已被安全策略拦截"}');
	}
	if(isset($_POST['login_apiurl'])){
		...
		if(!empty($_POST['login_apiurl']) && !ssrf_validate_url($_POST['login_apiurl']))exit('{"code":-1,"msg":"聚合登录API地址指向内网或保留地址，已被安全策略拦截"}');
	}
	foreach($_POST as $k=>$v){
		saveSetting($k, $v);
	}
	$ad=$CACHE->clear();
	if($ad){ @$DB->insert('log', ['uid'=>0, 'type'=>'修改系统配置', 'date'=>'NOW()', 'ip'=>$clientip, 'city'=>'']); exit('{"code":0,"msg":"succ"}'); }
	else safe_error_log('db_error', $DB->error());
	exit('{"code":-1,"msg":"修改设置失败"}');
break;
case 'setGonggao':
	$id=intval($_GET['id']);
...
case 'article_upload':
	...
	// B2-010: MIME 类型验证和文件大小限制
	$allowed_mime = ['image/gif', 'image/jpeg', 'image/png', 'image/bmp', 'image/webp'];
	$file_mime = function_exists('finfo_file') ? finfo_file(finfo_open(FILEINFO_MIME_TYPE), $tmp_name) : $_FILES['imgFile']['type'];
	if(!in_array($file_mime, $allowed_mime)){
		exit('{"error":1,"message":"上传文件类型不允许，仅支持图片文件。"}');
	}
