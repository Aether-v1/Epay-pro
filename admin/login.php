...
if(isset($_GET['act']) && $_GET['act']=='login'){
  ...
  // A1: 使用 password_verify 验证，支持旧明文密码自动迁移
  $pwdOk = false;
  ...
  if($username == $conf['admin_user']){
    if(strpos($storedPwd, '$2y$') === 0 || strpos($storedPwd, '$2a$') === 0 || strpos($storedPwd, '$argon2') === 0){
      // 现代密码哈希
      $pwdOk = password_verify($password, $storedPwd);
    } elseif($storedPwd === $password && $storedPwd !== '!INSTALL_REQUIRED!'){
      // 旧明文密码：验证成功后自动迁移为 password_hash
      $pwdOk = true;
      $newHash = password_hash($password, PASSWORD_DEFAULT);
      $DB->exec("UPDATE pre_config SET v='" . $newHash . "' WHERE k='admin_pwd'");
  ...
  if($pwdOk){
    ...
    if (file_exists($login_limit_file)) {
      unlink($login_limit_file);
		...
		$session=md5($username.substr($storedPwd,0,20).$password_hash); // A1: 不使用明文密码
		$expiretime=time() + 2592000;
		$token=authcode("{$username}\t{$session}\t{$expiretime}", 'ENCODE', SYS_KEY);
		session_regenerate_id(true); // A1: 登录成功轮换 Session ID
  ...
  // Batch C Phase2: TOTP 登录保存 admin_id
  $admin_id = 0;
  try {
      $admin_row = $DB->getRow("SELECT id, status FROM pre_admins WHERE username = ?", [$conf['admin_user']]);
      if ($admin_row && $admin_row['status'] == 1) {
          $admin_id = intval($admin_row['id']);
          $DB->exec("UPDATE pre_admins SET last_login_at = NOW() WHERE id = ?", [$admin_id]);
      }
  } catch (Exception $e) {}
  $_SESSION['admin_id'] = $admin_id;
  
  $DB->insert('log', ['uid'=>$admin_id, 'type'=>'登录后台', 'date'=>'NOW()', 'ip'=>$clientip]);
  ...
  exit(json_encode(['code'=>0]));
	...
	// Batch C Phase2: 清除 admin_id Session
