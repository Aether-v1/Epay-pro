...
case 'setPayType':
	...
	else safe_error_log('修改支付方式失败', $DB->error());
		exit('{"code":-1,"msg":"修改支付方式失败"}');
break;
case 'delPayType':
	$id=intval($_GET['id']);
	$row=$DB->getRow("select * from pre_type where id='$id' limit 1");
	...
	else safe_error_log('删除支付方式失败', $DB->error());
		exit('{"code":-1,"msg":"删除支付方式失败"}');
break;
case 'savePayType':
	if($_POST['action'] == 'add'){
		...
		else safe_error_log('新增支付方式失败', $DB->error());
		exit('{"code":-1,"msg":"新增支付方式失败"}');
	}else{
		$id=intval($_POST['id']);
		$name=trim($_POST['name']);
		$showname=trim($_POST['showname']);
...
case 'setChannel':
	...
	else safe_error_log('修改支付通道失败', $DB->error());
		exit('{"code":-1,"msg":"修改支付通道失败"}');
break;
case 'delChannel':
	// Batch C Phase2: 删除支付通道权限检查和审计
	require_permission('payment.channel');
	admin_audit_log('删除支付通道', json_encode(['id'=>$_POST['id'] ?? '']));
	$id=intval($_GET['id']);
	$row=$DB->getRow("SELECT * FROM pre_channel WHERE id='$id'");
	if(!$row)
		exit('{"code":-1,"msg":"当前支付通道不存在！"}');
	...
	else safe_error_log('删除支付通道失败', $DB->error());
		exit('{"code":-1,"msg":"删除支付通道失败"}');
break;
case 'saveChannel':
	if($_POST['action'] == 'add'){
		$name=trim($_POST['name']);
		...
		else safe_error_log('新增支付通道失败', $DB->error());
		exit('{"code":-1,"msg":"新增支付通道失败"}');
		...
		else safe_error_log('复制支付通道失败', $DB->error());
		exit('{"code":-1,"msg":"复制支付通道失败"}');
		...
		if($DB->update('channel', $data, ['id'=>$id])!==false){
			@$DB->insert('log', ['uid'=>0, 'type'=>'修改支付通道', 'date'=>'NOW()', 'ip'=>$clientip, 'city'=>'', 'data'=>'channel_id='.$id]);
...
case 'saveChannelInfo':
	...
	// B2-008: 支付通道 appurl SSRF 防护
	if(!empty($config['appurl']) && !ssrf_validate_url($config['appurl'])){
		exit('{"code":-1,"msg":"支付通道API地址指向内网或保留地址，已被安全策略拦截"}');
	}
	$config = json_encode($config);
	$data = ['config'=>$config, 'apptype'=>$apptype, 'appwxmp'=>$appwxmp, 'appwxa'=>$appwxa];
	if($DB->update('channel', $data, ['id'=>$id])!==false){ @$DB->insert('log', ['uid'=>0, 'type'=>'修改支付密钥', 'date'=>'NOW()', 'ip'=>$clientip, 'city'=>'', 'data'=>'channel_id='.$id]); exit('{"code":0,"msg":"修改支付密钥成功！"}'); }
	else safe_error_log('修改支付密钥失败', $DB->error());
		exit('{"code":-1,"msg":"修改支付密钥失败"}');
break;
case 'getRoll':
	$id=intval($_GET['id']);
...
case 'setRoll':
	...
	else safe_error_log('修改轮询组失败', $DB->error());
		exit('{"code":-1,"msg":"修改轮询组失败"}');
break;
case 'delRoll':
	$id=intval($_GET['id']);
	$row=$DB->getRow("select * from pre_roll where id='$id' limit 1");
	...
	else safe_error_log('删除轮询组失败', $DB->error());
		exit('{"code":-1,"msg":"删除轮询组失败"}');
break;
case 'saveRoll':
	if($_POST['action'] == 'add'){
		$name=trim($_POST['name']);
		...
		else safe_error_log('新增轮询组失败', $DB->error());
		exit('{"code":-1,"msg":"新增轮询组失败"}');
	}else{
		$id=intval($_POST['id']);
		$name=trim($_POST['name']);
		$type=intval($_POST['type']);
...
case 'delWeixin':
	...
	}else safe_error_log('删除公众号/小程序失败', $DB->error());
		exit('{"code":-1,"msg":"删除公众号/小程序失败"}');
break;
case 'saveWeixin':
	if($_POST['action'] == 'add'){
		$type=intval($_POST['type']);
		...
		else safe_error_log('新增公众号/小程序失败', $DB->error());
		exit('{"code":-1,"msg":"新增公众号/小程序失败"}');
	}else{
		$id=intval($_POST['id']);
		$type=intval($_POST['type']);
		$name=trim($_POST['name']);
		...
		else safe_error_log('修改公众号/小程序失败', $DB->error());
		exit('{"code":-1,"msg":"修改公众号/小程序失败"}');
	}
break;
case 'testweixin':
	$id=intval($_POST['id']);
...
case 'setWework':
	...
	else safe_error_log('修改企业微信失败', $DB->error());
		exit('{"code":-1,"msg":"修改企业微信失败"}');
break;
case 'delWework':
	$id=intval($_GET['id']);
	$row=$DB->getRow("select * from pre_wework where id='$id' limit 1");
	...
	}else safe_error_log('删除企业微信失败', $DB->error());
		exit('{"code":-1,"msg":"删除企业微信失败"}');
break;
case 'saveWework':
	if($_POST['action'] == 'add'){
		$name=trim($_POST['name']);
		...
		else safe_error_log('新增企业微信失败', $DB->error());
		exit('{"code":-1,"msg":"新增企业微信失败"}');
	}else{
		$id=intval($_POST['id']);
		$name=trim($_POST['name']);
		$appid=trim($_POST['appid']);
