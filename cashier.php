...
// XSS 转义 helper（仅用于 HTML 输出，不改变业务数据）
...
<title>收银台 | <?php echo epay_esc($sitename?$sitename:$conf['sitename'])?> </title>
...
<div class="w1080 order-amount12">
    <ul class="order-amount12-left">
        <li>
            <span>商品名称：</span>
            <span><?php echo epay_esc($row['name'])?></span>
        </li>
        <li>
            <span>订单号：</span>
            <span><?php echo epay_esc($trade_no)?></span>
        </li>
		<li>
            <span>创建时间：</span>
            <span><?php echo epay_esc($row['addtime'])?></span>
        </li>
    </ul>
    <div class="order-amount12-right">
        <span>订单金额：</span>
        <strong><?php echo epay_esc($row['money'])?></strong>
        <span>元</span>
    </div>
</div>
<?php }?>
...
<div class="w1080 PayMethod12">
    <div class="row">
		...
		<?php foreach($paytype as $rows){?>
          <li class="pay_li" value="<?php echo epay_esc($rows['id'])?>">
             <img src="/assets/icon/<?php echo epay_esc($rows['name'])?>.ico">
                    <span><?php echo epay_esc($rows['showname'])?></span>
          </li>
		<?php }?>
        </ul>
    </div>
...
<div class="w1080 immediate-pay12">
  <div class="immediate-pay12-right">
      <span>需支付：<strong><?php echo epay_esc($payamount)?></strong>元<?php if($has_fee)echo '（包含'.epay_esc($row['realmoney']-$row['money']).'元手续费）';?></span>
...
<div class="w1080 footer12">
    <p> <?php echo epay_esc($sitename?$sitename:$conf['sitename'])?></p>
</div>
