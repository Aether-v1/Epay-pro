    ...
    <body class="epay-page">
        <div class="epay-card">
            <div class="epay-status epay-status--error">
                ...
                <h1 class="epay-status__title"><?php echo $safe_title?></h1>
                <div class="epay-status__desc epay-msg-body"><?php echo $msg?></div>
                <div class="epay-status__actions">
                    <a href="javascript:history.back(-1)" class="epay-btn epay-btn--ghost">返回</a>
                </div>
            </div>
        </div>
    </body>
...
function returnTemplate($return_url) {
    ...
    <body class="epay-page">
        ...
        <script>
        (function(){
            var u = window.atob("<?php echo $url?>");
            var a = document.getElementById('epay-return-link');
            if(a){ a.onclick = function(){ window.location.href = u; return false; }; }
        })();
        </script>
    </body>
    </html>
...
function changeUserMoney($uid, $money, $add=true, $type=null, $orderid=null){
	...
	$DB->beginTransaction();
	// P0: 同时查询 money 和 frozen_money，使用行锁
	$userRow = $DB->getRow("SELECT money, frozen_money FROM pre_user WHERE uid=:uid LIMIT 1 FOR UPDATE", [':uid'=>$uid]);
	$oldmoney = $userRow['money'];
	$frozen = isset($userRow['frozen_money']) ? (float)$userRow['frozen_money'] : 0;
	if($add == true){
		$action = 1;
		$newmoney = round($oldmoney+$money, 2);
	}else{
		// P0: 减少资金时检查可用余额 = money - frozen_money
		$available = round($oldmoney - $frozen, 2);
		if($money > $available){
			$DB->rollBack();
			return false;
		}
...
function changeUserMoney2($uid, $oldmoney, $money, $add=true, $type=null, $orderid=null){
	...
	if($add == true){
		...
		// P0: 减少资金时查询 frozen_money 并检查可用余额
		$frozen = $DB->getColumn("SELECT frozen_money FROM pre_user WHERE uid=:uid LIMIT 1", [':uid'=>$uid]);
		$frozen = $frozen ? (float)$frozen : 0;
		$available = round($oldmoney - $frozen, 2);
		if($money > $available){
			return false;
		}
 ...
 * E3-A.2: 冻结商户资金（转账预留）
 * 
 * 在事务内执行：锁定用户行 → 检查可用余额 → 增加 frozen_money
 * 可用余额 = money - frozen_money
 * 
 * @param int $uid 商户ID
 * @param float $amount 冻结金额
 * @param string $biz_no 业务编号（用于审计）
 * @return bool 是否成功
 */
function freezeUserMoney($uid, $amount, $biz_no = null){
    global $DB;
    ...
    $money = (float)$userRow['money'];
    $frozen = isset($userRow['frozen_money']) ? (float)$userRow['frozen_money'] : 0;
    $available = round($money - $frozen, 2);
    ...
    $newFrozen = round($frozen + $amount, 2);
    $res = $DB->exec("UPDATE pre_user SET frozen_money=:frozen WHERE uid=:uid", [':frozen'=>$newFrozen, ':uid'=>$uid]);
    
    // 记录资金流水（冻结不改变 money，但记录审计）
    $DB->insert('record', ['uid'=>$uid, 'action'=>3, 'money'=>$amount, 'oldmoney'=>$money, 'newmoney'=>$money, 'type'=>'转账冻结', 'trade_no'=>$biz_no, 'date'=>'NOW()']);
...
/**
 * E3-A.2: 解冻商户资金（转账释放/拒绝）
 * 
 * 在事务内执行：锁定用户行 → 减少 frozen_money
 * 
 * @param int $uid 商户ID
 ...
 * @return bool 是否成功
 */
function unfreezeUserMoney($uid, $amount, $biz_no = null){
    global $DB;
    if($amount <= 0) return false;
    ...
    // 记录资金流水（解冻不改变 money，但记录审计）
    $DB->insert('record', ['uid'=>$uid, 'action'=>4, 'money'=>$amount, 'oldmoney'=>$money, 'newmoney'=>$money, 'type'=>'转账解冻', 'trade_no'=>$biz_no, 'date'=>'NOW()']);
...
/**
 * E3-A.2: 结算冻结资金（转账成功，将冻结转为正式扣减）
 * 
 * 在事务内执行：锁定用户行 → 减少 money → 减少 frozen_money
 * 这是原子操作，不会出现"已扣money但未减frozen"的中间状态。
 * 
 * @param int $uid 商户ID
 ...
 * @return bool 是否成功
 */
function settleFrozenUserMoney($uid, $amount, $biz_no = null){
    global $DB;
    if($amount <= 0) return false;
    ...
    $newmoney = round($oldmoney - $amount, 2);
    $newFrozen = round($frozen - $amount, 2);
    
    // 原子更新：同时减少 money 和 frozen_money
    $res = $DB->exec("UPDATE pre_user SET money=:money, frozen_money=:frozen WHERE uid=:uid", [
        ':money' => $newmoney,
        ':frozen' => $newFrozen,
        ':uid' => $uid
    ]);
    ...
    // 记录资金流水
    $DB->insert('record', ['uid'=>$uid, 'action'=>2, 'money'=>$amount, 'oldmoney'=>$oldmoney, 'newmoney'=>$newmoney, 'type'=>'代付', 'trade_no'=>$biz_no, 'date'=>'NOW()']);
...
// Batch B: SSRF 防护
// ============================================================
function ssrf_validate_url($url, $allowed_schemes = ['http', 'https']){
    ...
    if(!in_array($scheme, $allowed_schemes)) return false;
    ...
    if(filter_var($host, FILTER_VALIDATE_IP)){
        return ssrf_validate_ip($host);
    }
    
    // 域名：解析 DNS 后校验 IP
    $ips = gethostbynamel($host);
    if(empty($ips)) return false; // DNS 解析失败
    
    foreach($ips as $ip){
        if(!ssrf_validate_ip($ip)) return false;
    }
    
    return true;
}

function ssrf_validate_ip($ip){
    // 禁止 loopback
    if(in_array($ip, ['127.0.0.1', '::1', '0.0.0.0'])) return false;
    ...
    // 禁止云 metadata 地址
    if($ip === '100.100.100.200') return false; // 阿里云
    if($ip === '169.254.169.254') return false; // AWS/通用
    
    return true;
}

function ssrf_safe_get_curl($url, $post=0, $referer=0, $cookie=0, $header=0, $ua=0, $nobaody=0, $addheader=0, $location=0){
    if(!ssrf_validate_url($url)){
        return false;
    }
    return get_curl($url, $post, $referer, $cookie, $header, $ua, $nobaody, $addheader, $location);
}

// ============================================================
// Batch C: 统一安全错误日志机制
// ============================================================
function safe_error_log($action, $error, $context = []) {
    // 脱敏：不记录密码、密钥、token
    $sensitive_keys = ['password', 'pwd', 'passwd', 'secret', 'token', 'api_key', 'apikey', 'private_key', 'csrf_token', 'session'];
    $safe_context = [];
    foreach ($context as $k => $v) {
        if (in_array(strtolower($k), $sensitive_keys)) {
            $safe_context[$k] = '***REDACTED***';
        } else {
            $safe_context[$k] = is_string($v) ? substr($v, 0, 500) : $v;
        }
    }
    
    $log_entry = sprintf(
        "[%s] ACTION=%s ERROR=%s CONTEXT=%s IP=%s URI=%s\n",
        date('Y-m-d H:i:s'),
        $action,
        is_string($error) ? substr($error, 0, 1000) : var_export($error, true),
    ...
    // 记录到 PHP 错误日志
    @error_log($log_entry, 3, SYSTEM_ROOT . 'logs/security_error.log');
    
    // 同时记录到数据库（如果可用）
    try {
        global $DB;
        if ($DB && is_object($DB)) {
            $uid = isset($_SESSION['uid']) ? intval($_SESSION['uid']) : (isset($_SESSION['admin_id']) ? 0 : 0);
            @$DB->insert('log', [
                'uid' => $uid,
                'type' => '安全错误',
                'date' => 'NOW()',
                ...
                'city' => '',
                'data' => substr($action . ': ' . (is_string($error) ? $error : ''), 0, 65535)
            ]);
        }
    } catch (Exception $e) {
        // 数据库记录失败不影响主流程
...
// 安全错误响应：不泄露内部错误给用户
function safe_error_response($action, $error, $user_msg = '操作失败，请稍后重试') {
    safe_error_log($action, $error);
    return $user_msg;
}

// ============================================================
// Batch C Phase2: RBAC 权限管理核心函数
// ============================================================
...
function get_current_admin_id() {
    ...
    // 旧模型：没有 admin_id，返回 0 表示超级管理员（兼容）
    return 0;
}
...
function get_admin_permissions($admin_id = null) {
    ...
    // 旧模型兼容：admin_id=0 表示超级管理员，拥有所有权限
    if ($admin_id == 0) {
        return ['*'];
    }
    ...
    try {
        $permissions = $DB->getAll("
            ...
            FROM pre_admin_roles ar
            JOIN pre_role_permissions rp ON ar.role_id = rp.role_id
            WHERE ar.admin_id = ?
        ", [$admin_id]);
        $perm_keys = array_column($permissions, 'permission_key');
    } catch (Exception $e) {
        $perm_keys = [];
    }
    
    $cache[$admin_id] = $perm_keys;
    return $perm_keys;
}

// 检查管理员是否拥有指定权限
function has_permission($permission_key, $admin_id = null) {
...
// 要求权限，否则拒绝
function require_permission($permission_key, $message = '权限不足') {
    if (!has_permission($permission_key)) {
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH'] === 'XMLHttpRequest') {
            exit(json_encode(['code' => 403, 'msg' => $message]));
        } else {
...
// 记录管理员操作审计日志
function admin_audit_log($action, $detail = '', $admin_id = null) {
    global $DB, $clientip;
    if ($admin_id === null) {
        $admin_id = get_current_admin_id();
    }
