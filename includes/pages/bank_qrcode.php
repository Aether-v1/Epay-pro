<?php
// 银联云闪付扫码支付页面（Phase 2-F1：统一二维码组件，云闪付 APP 拉起逻辑保留）
...
<body class="epay-page epay-qr-page epay-channel--bank">
<div class="epay-card">
  <div class="epay-head">
  ...
  <h1 class="epay-qr-page__title"><span class="epay-channel-ico" aria-hidden="true"><svg viewBox="0 0 24 24"><text x="12" y="16" text-anchor="middle" font-size="12" font-weight="700" fill="#ffffff" font-family="inherit">闪</text></svg></span>银联云闪付扫码支付</h1>
  <p class="epay-qr-page__subtitle">使用云闪付 App 扫一扫完成付款</p>
  ...
  <div class="epay-qr">
    ...
    <div class="epay-qr__help">请使用云闪付 App 扫描二维码完成支付</div>
  </div>

  <div class="open_app" style="display:none">
    <a class="btn-open-app">打开云闪付APP继续付款</a>
...
<script>
    ...
    if(code_type == 0){
        ...
        $('#qrcode').empty().append($('<img>', {src: code_url, alt: '云闪付收款二维码'}));
    }
    // 订单详情
    ...
    function loadmsg() {
        $.ajax({
            ...
            dataType: "json",
            url: "/getshop.php",
            data: {type: "bank", trade_no: trade_no},
